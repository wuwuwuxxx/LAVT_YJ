...
def criterion(output, target, cls_tar):
    weight = torch.FloatTensor([0.9, 1.1]).cuda()
    weight_mask = 1 + cls_tar
    loss2 = nn.functional.cross_entropy(output, target, weight=weight, reduction='none')
...
def train_one_epoch(args, model, criterion, optimizer, data_loader, lr_scheduler, epoch, print_freq,
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        total_its += 1
        image, target, sentences, attentions, sentences_len, _, _, _, cls_tar = data
        ...
        loss = args.loss_weight * criterion(output, target, cls_tar)
