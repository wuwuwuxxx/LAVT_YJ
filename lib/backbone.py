...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask, g_fea):
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        self.cost_aggre = False
        # 加cost_aggre：
        if cost_aggre:
            self.cost_aggre = cost_aggre

            # 最终encode特征映射层
            self.corr_conv1 = nn.Conv2d(dim, int(dim/1), kernel_size=7, stride=1, padding=3)
            self.guidance_projection = nn.Sequential(
                nn.Conv2d(dim, aggregation_dim, kernel_size=3, stride=1, padding=1),
                nn.ReLU(),
            ) 
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                out = x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous().detach()
                outs.append(out)
    ...
    def train(self, mode=True):
        ...
        self._freeze_stages()
class Cost_WindowAttention(nn.Module):
...
class COST_SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = norm_layer(dim)
        self.attn = Cost_WindowAttention(
...
class SwinTransformerBlockWrapper(nn.Module):
    def __init__(self, dim, appearance_guidance_dim, input_resolution, nheads=4, window_size=5):
        super().__init__()
        self.block_1 = COST_SwinTransformerBlock(dim, appearance_guidance_dim, input_resolution, num_heads=nheads, head_dim=None, window_size=window_size, shift_size=0)
        self.block_2 = COST_SwinTransformerBlock(dim, appearance_guidance_dim, input_resolution, num_heads=nheads, head_dim=None, window_size=window_size, shift_size=window_size // 2)
    ...
    def forward(self, x, appearance_guidance):
        ...
        if appearance_guidance is not None:
            appearance_guidance = self.guidance_norm(rearrange(appearance_guidance, 'B C H W -> B (H W) C'))
...
class AggregatorLayer(nn.Module):
    def __init__(self, hidden_dim=64, appearance_guidance=512, nheads=4, input_resolution=(15, 15), window_size=5) -> None:
        super().__init__()
